...
function Dashboard({ session, onShowPricing, isSuccess }) {
  ...
  const addReview = async () => {
    ...
    const { data, error } = await supabase.from('reviews').insert([{ author: newReview.author, rating: newReview.rating, text: newReview.text, platform: newReview.platform, replied: false, user_id: session.user.id }]).select()
    if (!error) {
      setReviews(prev => [{ ...data[0], aiReply: null, loading: false, manualReply: undefined, showManual: false }, ...prev])
      setNewReview({ author: '', rating: 5, text: '', platform: 'Google' }); setShowForm(false)
    }
  }

  const stars = (rating) => Array.from({ length: 5 }, (_, i) => <span key={i} style={{ color: i < rating ? '#6366f1' : '#e5e7eb' }}>★</span>)
  ...
  const getAvatarColor = (name) => { const i = name.charCodeAt(0) % avatarColors.length; return { bg: avatarColors[i], text: avatarTextColors[i] } }
  const platformStyles = { 'Google': { bg: '#e0e7ff', color: '#4338ca' }, 'TripAdvisor': { bg: '#d1fae5', color: '#065f46' }, 'Facebook': { bg: '#dbeafe', color: '#1d4ed8' } }
    ...
    <div className="min-h-screen flex" style={{ backgroundColor: '#f8fafc' }}>
      ...
      <main className="md:ml-56 flex-1 px-4 md:px-8 py-5 md:py-8 mt-12 md:mt-0">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-1.5 text-white text-xs font-medium px-3.5 py-2 rounded-lg" style={{ backgroundColor: '#6366f1' }}>
        ...
        {showForm && (
          <div className="bg-white rounded-xl border p-5 mb-5" style={{ borderColor: '#f1f5f9' }}>
            ...
            <div className="space-y-3">
              <input type="text" placeholder="Author name" value={newReview.author} onChange={e => setNewReview({ ...newReview, author: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" style={{ borderColor: '#e5e7eb' }} />
              <textarea placeholder="Review text" value={newReview.text} onChange={e => setNewReview({ ...newReview, text: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm h-20 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none" style={{ borderColor: '#e5e7eb' }} />
              <div className="flex gap-3">
                <select value={newReview.rating} onChange={e => setNewReview({ ...newReview, rating: parseInt(e.target.value) })} className="border rounded-lg px-3 py-2 text-sm" style={{ borderColor: '#e5e7eb' }}>
                  {[1,2,3,4,5].map(n => <option key={n} value={n}>{n} stars</option>)}
                </select>
                <select value={newReview.platform} onChange={e => setNewReview({ ...newReview, platform: e.target.value })} className="border rounded-lg px-3 py-2 text-sm" style={{ borderColor: '#e5e7eb' }}>
                  <option>Google</option><option>TripAdvisor</option><option>Facebook</option>
                </select>
              </div>
              <div className="flex gap-2">
                <button onClick={addReview} className="text-white text-sm px-4 py-2 rounded-lg" style={{ backgroundColor: '#6366f1' }}>Save review</button>
                <button onClick={() => setShowForm(false)} className="text-gray-400 text-sm px-4 py-2 rounded-lg">Cancel</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl border" style={{ borderColor: '#f1f5f9' }}>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12"><div className="w-6 h-6 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"></div></div>
          ) : reviews.length === 0 ? (
                  ...
                  <div key={review.id} className="px-5 py-4 hover:bg-gray-50 transition-colors">
                    <div className="flex items-start justify-between mb-2.5">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium flex-shrink-0" style={{ backgroundColor: av.bg, color: av.text }}>
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className="text-xs px-2 py-0.5 rounded-md font-medium" style={{ backgroundColor: plat.bg, color: plat.color }}>{review.platform}</span>
                          </div>
                        </div>
                      </div>
                      {review.replied ? (
                    ...
                    {review.aiReply && (
                      <div className="ml-10 rounded-xl p-3.5 mb-3" style={{ backgroundColor: '#eef2ff' }}>
                        ...
                        <button onClick={() => markAsReplied(review.id)} className="mt-3 text-white text-xs px-3 py-1.5 rounded-lg" style={{ backgroundColor: '#16a34a' }}>✓ Mark as replied</button>
                      </div>
                    )}
                    {review.showManual && (
                      <div className="ml-10 mt-2">
                        <textarea value={review.manualReply || ''} onChange={e => setReviews(prev => prev.map(r => r.id === review.id ? { ...r, manualReply: e.target.value } : r))}
                          placeholder="Write your reply..." className="w-full border rounded-xl px-3 py-2.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none" style={{ borderColor: '#e5e7eb' }} rows={3} />
                        <div className="flex gap-2 mt-2">
                          <button onClick={() => markAsReplied(review.id)} className="text-white text-xs px-3 py-1.5 rounded-lg" style={{ backgroundColor: '#16a34a' }}>✓ Mark as replied</button>
                          <button onClick={() => setReviews(prev => prev.map(r => r.id === review.id ? { ...r, showManual: false, manualReply: undefined } : r))} className="text-gray-400 text-xs px-3 py-1.5 rounded-lg">Cancel</button>
                        </div>
                      </div>
                    )}
                    {!review.replied && !review.aiReply && !review.showManual && (
                      <div className="ml-10 flex items-center gap-2">
                        <button onClick={() => generateReply(review)} disabled={review.loading} className="text-xs px-3 py-1.5 rounded-lg border transition-colors disabled:opacity-50 flex items-center gap-1.5" style={{ borderColor: '#e0e7ff', color: '#6366f1', backgroundColor: '#fafafa' }}>
                        ...
                        <button onClick={() => setReviews(prev => prev.map(r => r.id === review.id ? { ...r, showManual: true, manualReply: '' } : r))} className="text-xs px-3 py-1.5 rounded-lg border transition-colors flex items-center gap-1.5" style={{ borderColor: '#e5e7eb', color: '#64748b', backgroundColor: '#fafafa' }}>
