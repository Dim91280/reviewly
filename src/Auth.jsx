      ...
      <div className="bg-white rounded-2xl border p-8 w-full max-w-md" style={{ borderColor: '#f1f5f9' }}>
        ...
        <div className="space-y-3">
          <input type="email" placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)}
            className="w-full border rounded-xl px-4 py-2.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            style={{ borderColor: '#e5e7eb' }} />
          <input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)}
            className="w-full border rounded-xl px-4 py-2.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            style={{ borderColor: '#e5e7eb' }} />
        </div>
        {message && <p className="text-sm mt-4" style={{ color: '#6366f1' }}>{message}</p>}
        <button onClick={handleSubmit} disabled={loading}
          className="w-full text-white font-medium py-2.5 rounded-xl mt-5 text-sm disabled:opacity-50"
        ...
        <p className="text-center mt-3">
          <button onClick={onBack} className="text-gray-400 text-xs hover:text-gray-600">Back to home</button>
        </p>
      </div>
    </div>
  )
